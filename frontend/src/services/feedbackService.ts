...
export const feedbackService = {
    submitFeedback: async (customerId: string, data: FeedbackRequest): Promise<Feedback> => {
        ...
        const response = await api.post(`/customers/${customerId}/feedback`, requestData);        return response.data.data;
    },
    ...
    getFeedback: async (customerId: string, feedbackId: number): Promise<Feedback> => {
        const response = await api.get(`/customers/${customerId}/feedback/${feedbackId}`);
        return response.data.data;
    },

    updateFeedback: async (customerId: string, feedbackId: number, data: FeedbackUpdateRequest): Promise<Feedback> => {
        const response = await api.patch(`/customers/${customerId}/feedback/${feedbackId}`, data);
        return response.data.data;
    },

    deleteFeedback: async (customerId: string, feedbackId: number): Promise<void> => {
        await api.delete(`/customers/${customerId}/feedback/${feedbackId}`);
    },

    getAverageRating: async (customerId: string): Promise<number> => {
        const response = await api.get(`/customers/${customerId}/feedback/average-rating`);
