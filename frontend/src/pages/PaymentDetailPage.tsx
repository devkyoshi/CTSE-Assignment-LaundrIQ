...
export default function PaymentDetailPage() {
  ...
  if (loading) {
    return <AppLayout><div className="space-y-4 max-w-2xl"><Skeleton className="h-8 w-48" /><Skeleton className="h-48 w-full" /></div></AppLayout>;
  }

  if (!payment) {
    return <AppLayout><div className="text-center py-12"><p className="text-muted-foreground">Payment not found</p></div></AppLayout>;
  }
    ...
    <AppLayout>
      <div className="space-y-6 max-w-2xl">
        <div className="flex items-center gap-3">
          <Button variant="ghost" size="icon" asChild><Link to="/payments"><ArrowLeft className="h-4 w-4" /></Link></Button>
          <h1 className="text-3xl font-bold tracking-tight text-foreground">Payment #{payment.id}</h1>
        </div>
        <Card>
          ...
          <CardContent>
            <div className="grid grid-cols-2 gap-4 text-sm">
              ...
              <div><span className="text-muted-foreground">Amount</span><p className="font-medium tabular-nums mt-0.5">{formatCurrency(payment.amount)}</p></div>
